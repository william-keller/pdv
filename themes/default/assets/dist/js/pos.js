...
function numberToReal(numero) {
    numero = numero.toString().toFixed(2).split('.');
    numero[0] = numero[0].split(/(?=(?:...)*$)/).join('.');
    return numero.join(',');
}

function numberToIng(numero) {
...
$(document).ready(function(){
    ...
    $(document).on('change', '#nPrice, #nDiscount', function () {
        ...
        var unit_price = parseFloat(formatReal($('#nPrice').val()));
        var net_price = unit_price;
        var item = spositems[item_id];
        var ds = formatReal($('#nDiscount').val()) ? formatReal($('#nDiscount').val()) : '0';
        item_discount = formatDecimal(parseFloat(ds));
        if (ds.indexOf("%") !== -1) {
            var pds = ds.split("%");
            if (!isNaN(pds[0])) {
    ...
    ============================= */
     $(document).on('click', '#editItem', function () {
        var item_id = $('#item_id').val();
        var price = parseFloat(formatReal($('#nPrice').val()));
        if(!is_valid_discount(formatReal($('#nDiscount').val()))) {
            bootbox.alert(lang.unexpected_value);
            return false;
        }
        spositems[item_id].row.qty = parseFloat($('#nQuantity').val()),
        spositems[item_id].row.real_unit_price = formatMoney(price),
        spositems[item_id].row.discount = formatReal($('#nDiscount').val()) ? formatReal($('#nDiscount').val()) : '0',
    ...
    $("#updateDiscount").click(function() {
        var ds = formatReal($('#get_ds').val()) ? formatReal($('#get_ds').val()) : '0';
    ...
    $("#add_discount").click(function() {
        ...
        $('#get_ds').val(formatMoney(dval));
        $('#dsModal').modal({backdrop:'static'});
        return false;
    });

    $("#updateTax").click(function() {
        var ts = formatReal($('#get_ts').val());
        if (ts.length != 0) {
            $('#tax_val').val(formatMoney(ts));
            store('spos_tax', ts);
            if(ts.indexOf("%") !== -1) {
                var pts = ts.split("%");
                if(! isNaN(pts[0])){
    ...
    $('#amount').on('keyup keydown change', function(e) {   
   var total_paying = formatReal($(".amount").val());
      
        $('#total_paying').text(formatMoney(total_paying));
        if(Settings.rounding != 0) {
            $('#balance').text(formatMoney(round_total-total_paying));
            $('#balance_val').val(formatDecimal(round_total-total_paying));
            total_paid = total_paying;
            grand_total = round_total;
        } else {
            $('#balance').text(formatMoney(total_paying-gtotal));
            $('#balance_val').val(total_paying-gtotal);
            total_paid = total_paying;
            grand_total = gtotal;
        }
    ...
    $('#payModal').on('change', '#paid_by', function () {
        ...
        if (p_val == 'cash' || p_val == 'other') {
            ...
            setTimeout(function(){ $('#valorcartao').focus(); }, 10);
      } 
        else if (p_val == 'CARTAO') {
            $('#valorcartao_div').show();
            $('.pcheque').hide('fast');
            $('.pcash').hide('fast');
          $('.ppc').hide('fast');
            //$('#amount').val(rounded_total);
            setTimeout(function(){ $('#valorcartao_in').focus(); }, 10);
      } 
        ...
        if (cn != '') {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    } else if (data.balance <= 0 || data.balance < gtotal) {
                        $('#amount').val(0);
                        $('#gift_card_no').parent('.form-group').addClass('has-error');
                        $('#gc_details').html(lang.value+' Inicial: ' + formatMoney(data.value) + ' / Saldo Atual: ' + formatMoney(data.balance));
                        bootbox.alert("Saldo insuficiente para o pagamento");
                    } else {
                        $('#gc_details').html(lang.value+' Inicial: ' + formatMoney(data.value) + ' / Saldo Atual: ' + formatMoney(data.balance));
    ...
    $('#payModal').on('keyup keydown', '.amount', function (e) {
        $('#amount_val').val(formatReal($(this).val()));
  
    });
  
   $('#payModal').on('keyup keydown', '#valorcartao_in', function (e) {
        $('#valorcartao_val').val(formatReal($(this).val()));
    });
    ...
    $('#submit-sale').click(function(event) {
        event.preventDefault();
        $('#amount').trigger("click");
      if($("#paid_by").val()=='gift_card'  && $("#amount").val()!=' '  && $("#amount").val()!='' && $("#amount").val()!='0,00' && $("#amount").val()!='0' && $("#amount").val()!='0.00'){
        ...
        event.preventDefault();
        $('#gift_card_no').parent('.form-group').addClass('has-error');
        bootbox.alert("Verifique o pagamento."); 
